...
public sealed class LoomException : PowerShellException {
  ...
  internal static LoomException HarnessNotInstalled(string name, string moduleName, Exception? innerException)
    => new(HARNESS_NOT_INSTALLED, ErrorCategory.ObjectNotFound,
      $"The harness '{name}' needs the module '{moduleName}', which is not installed. Install it with: " +
      $"Install-PSResource -Name {moduleName} -Scope CurrentUser", moduleName, innerException);
